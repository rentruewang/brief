...
from torch.nn import GRU, Embedding, GRUCell, Linear, Module
...
class Encoder(Module):
    def __init__(self, voc_size, hidden_size, num_layers):
        super().__init__()
        self.embedding = Embedding(num_embeddings=voc_size, embedding_dim=hidden_size)
        self.rnn = GRU(
...
class OneHotEncoder(Module):
    ...
    def __init__(self, voc_size, hidden_size, num_layers):
        super().__init__()
        self.embedding = Linear(in_features=voc_size, out_features=hidden_size)
        self.rnn = GRU(
            input_size=hidden_size, hidden_size=hidden_size, num_layers=num_layers
        )
...
class AttnDecoder(Module):
    def __init__(self, voc_size, hidden_size, timesteps, num_layers):
        ...
        self.combine = Linear(in_features=2 * hidden_size, out_features=hidden_size)
        self.rnn = GRU(
            input_size=hidden_size, hidden_size=hidden_size, num_layers=num_layers
        )
    ...
    def forward(self, input, states, gru_out):
        ...
        return self.rnn(combined.permute(1, 0, 2), states)


class Q_Generator(Module):
    """
    ...
    def __init__(
        ...
        self.average = Linear(in_features=hidden_size, out_features=1, bias=False)
...
class Reconstructor(Module):
    def __init__(
        ...
        self.decoder = AttnDecoder(voc_size, hidden_size, time_steps, num_layers)
        self.output = Linear(in_features=hidden_size, out_features=voc_size)
...
class Discriminator(Module):
    def __init__(self, voc_size, hidden_size, device, num_layers=2):
        ...
        self.score = Linear(in_features=hidden_size * num_layers, out_features=1)
...
if __name__ == "__main__":
    ...
    inp = torch.randint(voc_size, (timesteps, batch))
    print(dis(inp).shape)
